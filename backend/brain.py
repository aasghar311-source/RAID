...
4b. MULTI-TIMEFRAME CONFIRMATION: each asset now includes three trend labels:
   ...
   - Set tighter TP targets (2.0-2.5%, hard-capped at 2.5%)
...
async def _execute_brain_trades(
    ...
    for trade_spec in trades:
        ...
        # Cap TP distance at MAX_TP_DISTANCE_PCT (4.62% avg never reached — 0/314 hit).
        if entry_price > 0 and take_profit > 0:
            if direction in ("long", "yes"):
                _max_tp = entry_price * (1 + config.MAX_TP_DISTANCE_PCT)
                if take_profit > _max_tp:
                    log.info("BRAIN: cap TP %s %.6f → %.6f (max 2.5%%)", symbol, take_profit, _max_tp)
                    take_profit = _max_tp
            else:
                _min_tp = entry_price * (1 - config.MAX_TP_DISTANCE_PCT)
                if take_profit < _min_tp:
                    log.info("BRAIN: cap TP %s %.6f → %.6f (max 2.5%%)", symbol, take_profit, _min_tp)
                    take_profit = _min_tp
        ...
        for sig in pending:
            ...
            if _trig > 0 and _sl > 0 and _tp > 0:
                ...
                if _risk > 0 and (_reward / _risk) < 1.25:
                    ...
                    _tp = _new_tp
                # Cap TP distance at MAX_TP_DISTANCE_PCT (applies after any widening).
                if _dir in ("long", "yes"):
                    _max_tp = _trig * (1 + config.MAX_TP_DISTANCE_PCT)
                    if _tp > _max_tp:
                        log.info("PENDING: cap TP %s %.6f → %.6f (max 2.5%%)", sig.get("symbol", "?"), _tp, _max_tp)
                        _tp = _max_tp
                        sig["take_profit"] = _max_tp
                else:
                    _min_tp = _trig * (1 - config.MAX_TP_DISTANCE_PCT)
                    if _tp < _min_tp:
                        log.info("PENDING: cap TP %s %.6f → %.6f (max 2.5%%)", sig.get("symbol", "?"), _tp, _min_tp)
                        _tp = _min_tp
                        sig["take_profit"] = _min_tp
            filtered.append(sig)
